...
/**
 * BEAST MODE MINER - 9950X3D Edition 🔥
 *
 * Optimized for high-end systems with 16+ cores
 *
 * Configuration:
 * - 4 parallel batches (vs 2 in gentle mode)
 * - 2000 preimages per batch (vs 1000)
 * - 4 hash-server instances on ports 3003-3006
 * - ~24/32 threads used (leaves 8 for desktop)
 *
 * Performance:
 * - 2-4x faster than gentle mode
 * - ~1000-2000 H/s on 9950X3D
 * - Completes all 16 addresses in ~60-120s
 *
 * Resource Usage:
 * - CPU: ~75% utilization
 * - RAM: ~8GB (4x hash-servers × 2GB each)
 * - Desktop remains responsive
 *
 * Usage:
 *   node beast-miner.js
 *
 * Compare to gentle mode:
 *   node parallel-miner.js  <- Quiet, ~2 cores, ~200-400 H/s
 *   node beast-miner.js     <- BEAST, ~24 cores, ~1500+ H/s
 */
...
const logger = new DualLogger('beast-miner.log');

// Configuration - BEAST MODE (9950X3D Edition 🔥)
const NUM_PARALLEL_BATCHES = 4; // 4 batches mining simultaneously
const ADDRESSES_PER_BATCH = 4;  // 4 addresses per batch
const TOTAL_ADDRESSES = NUM_PARALLEL_BATCHES * ADDRESSES_PER_BATCH; // 16 total
const WORKERS_PER_BATCH = 1; // 1 worker per batch (4 total active)
const BATCH_SIZE = 2000; // 2000 preimages/batch (vs 1000 in gentle mode)
const HASH_SERVER_BASE_PORT = 3003; // Each batch gets own hash-server on ports 3003-3006

logger.raw(chalk.red.bold('\n🔥🔥🔥 BEAST MODE MINER - 9950X3D EDITION 🔥🔥🔥\n'));
...
logger.log(chalk.magenta(`Batch size: ${BATCH_SIZE} preimages (2x aggressive)`));
logger.log(chalk.magenta(`Hash servers: ${NUM_PARALLEL_BATCHES}x instances on ports ${HASH_SERVER_BASE_PORT}-${HASH_SERVER_BASE_PORT + NUM_PARALLEL_BATCHES - 1}`));
logger.log(chalk.cyan(`💪 Using ~24/32 threads - Desktop still usable!\n`));
...
async function mineChallenge(challenge, address, batchId) {
  ...
  // Assign hash-server port per batch (each batch gets dedicated instance)
  const hashServerPort = HASH_SERVER_BASE_PORT + (batchId - 1);

  const solver = new AshMaizeSolver({
    numWorkers: WORKERS_PER_BATCH,
    batchSize: BATCH_SIZE,
    hashEnginePort: hashServerPort,
    address: address
  });
  ...
  while (true) {
    try {
      ...
      if (!solvedChallenges.has(challenge.id)) {
        ...
        const avgTimePerAddress = mined > 0 ? (parseFloat(totalTime) / mined).toFixed(2) : 'N/A';
        const totalHashes = flatResults.reduce((sum, r) => sum + (r.attempts || 0), 0);
        const hashRate = totalHashes > 0 ? ((totalHashes / parseFloat(totalTime))).toFixed(0) : 'N/A';
        ...
        logger.log(chalk.magenta(`   ⚡ BEAST STATS: ${hashRate} H/s | ${avgTimePerAddress}s/addr | ${totalHashes} total hashes`));
